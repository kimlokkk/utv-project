...
<body class="skin-green fixed-layout">
            ...
            <div class="container-fluid">
                ...
                <div class="row page-titles">
                    <div class="col-md-5 align-self-center">
                        <h4 class="text-themecolor">New Procurement Application</h4>
                    </div>
                                    ...
                                    <table class="table table-bordered table-striped" id="procurementTable">
                                        ...
                                        <tbody>
                                            <?php
                                                ...
                                                FROM project p
                                                INNER JOIN procurement pr ON p.id = pr.project_id
                                                WHERE pr.status = 'Pending Verification' OR pr.status = 'Pending Approval'
                                                ORDER BY procurement_id DESC"; 
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    <button type="button" class="btn btn-info btn-sm" id="verifyProcurement" 
                                                        data-procurement-id="<?php echo urlencode($procurement_id); ?>" 
                                                        data-project-id="<?php echo urlencode($project_id); ?>" 
                                                        data-project-no="<?php echo urlencode($project_no); ?>">
                                                        Verify
                                                    </button>&nbsp;
                                                    <button type="button" class="btn btn-success btn-sm" id="approveProcurement" 
                                                        data-procurement-id="<?php echo urlencode($procurement_id); ?>" 
                                                        data-project-id="<?php echo urlencode($project_id); ?>" 
                                                        data-project-no="<?php echo urlencode($project_no); ?>">
                                                        Approve
                                                    </button>&nbsp;
                                                    <button type="button" class="btn btn-danger btn-sm" id="rejectProcurement" 
                                                        data-procurement-id="<?php echo urlencode($procurement_id); ?>" 
                                                        data-project-id="<?php echo urlencode($project_id); ?>" 
                                                        data-project-no="<?php echo urlencode($project_no); ?>">
                                                        Reject
    ...
    <script>
        $(document).on('click', '#rejectProcurement', function () {
            Swal.fire({
                title: 'Are you sure?',
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire({
                        ...
                        if (remarkResult.isConfirmed) {
                            ...
                            Swal.fire(
                                'Cancelled',
                                'You need to provide a remark to reject the procurement.',
                                'info'
                            );
                        }
                    }).catch((error) => {
                    ...
                    Swal.fire(
                        'Cancelled',
                        'Procurement rejection has been cancelled.',
                        'info'
                    );
                }
            });
        ...
        $(document).on('click', '#approveProcurement', function () {
            Swal.fire({
                ...
                if (result.value) { // Jika pengguna mengesahkan
                    ...
                    Swal.fire(
                        'Cancelled',
                        'Procurement approval has been cancelled.',
                        'info'
                    );
                }
            });
    ...
    <script>
        $(document).on('click', '#verifyProcurement', function () {
            Swal.fire({
                title: 'Are you sure?',
