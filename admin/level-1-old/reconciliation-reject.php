<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') { // Gunakan POST
        // Validasi input
        if (empty($_POST['projectId']) || empty($_POST['applicationId']) || empty($_POST['projectNo']) || empty($_POST['remark']) || empty($_POST['applicationType'])) {
        ...
        try {
            ...
            $tracker_query = "INSERT INTO project_tracker (project_id, project_no, remark, date) 
                              VALUES ('$project_id', '$project_no', '$application_type application has been rejected by admin; $remark', '$date')";
