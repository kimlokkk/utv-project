<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') { // Gunakan POST
        // Validasi input
        if (empty($_POST['projectId']) || empty($_POST['procurementId']) || empty($_POST['projectNo']) || empty($_POST['remark'])) {
        ...
        $procurement_id = mysqli_real_escape_string($db, $_POST['procurementId']);
        ...
        try {
            ...
            $tracker_query = "INSERT INTO project_tracker (project_id, project_no, remark, date) 
                              VALUES ('$project_id', '$project_no', 'Procurement application has been rejected by admin; $remark', '$date')";
