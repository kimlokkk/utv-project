<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') { // Gunakan POST
        // Validasi input
        if (empty($_POST['projectId']) || empty($_POST['professionalId']) || empty($_POST['projectNo']) || empty($_POST['remark']) || empty($_POST['memberName'])) {
        ...
        try {
            ...
            $tracker_query = "INSERT INTO project_tracker (project_id, project_no, remark, date) 
                              VALUES ('$project_id', '$project_no', 'Professional fee application for $member_name has been rejected by admin; $remark', '$date')";
