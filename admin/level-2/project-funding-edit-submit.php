...
$app_query = "
    ...
    WHERE pfa.id = '$application_id'
      AND pfa.status = 'Pending Approval'
    LIMIT 1
";
$app_result = mysqli_query($db, $app_query);

if (!$app_result || mysqli_num_rows($app_result) === 0) {
    pfa_admin_edit_response(false, 'This project funding assistance application cannot be edited by Level 2.');
}
...
try {
    ...
    mysqli_query($db, "INSERT INTO project_tracker (project_id, project_no, remark, date) VALUES ('$project_id', '$project_no', 'Project funding assistance application has been edited by Level 2.', '$date_now')");

    mysqli_commit($db);
    pfa_admin_edit_response(true, 'Project funding assistance application updated successfully.');
} catch (Exception $e) {
    mysqli_rollback($db);
