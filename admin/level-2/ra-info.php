    ...
    <div id="main-wrapper">
        ...
        <div class="page-wrapper">
            ...
            <div class="container-fluid">
                ...
                $disableButtons = $isRejected || $isApproved;
                ?>
                <div class="row m-b-30">
                    <div class="col-md-12">
                        <button type="button" id="approveResearch" class="btn btn-lg btn-success"
                        ...
                        <button type="button" id="returnResearch" class="btn btn-lg btn-danger"
                            data-research-id="<?php echo $id; ?>"
                            data-admin-staff-id="<?php echo $userData['staff_id']; ?>"
                            <?php echo $disableButtons ? 'disabled style="opacity: 0.5; cursor: not-allowed;"' : ''; ?>>
                            Return
        ...
        $(document).on('click', '#approveResearch', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: 'ra-approve.php', // 👈 tukar URL jika berbeza
                        method: 'POST',
                        dataType: 'json',
                        data: {
                    ...
                    Swal.fire('Cancelled', 'Approval has been cancelled.', 'info');
                }
            });
        });
    </script>
    <script>
        $(document).on('click', '#returnResearch', function () {
            const researchId = $(this).data('research-id');
            const staffId = $(this).data('admin-staff-id');
    
            Swal.fire({
                title: 'Return Research Application?',
                html:
                    '<label for="return_to">Return To:</label><br>' +
                    '<select id="return_to" class="swal2-select" style="width: 100%; padding: 5px;">' +
                        '<option value="Level 3">CST Level 3</option>' +
                        '<option value="Research Assistant">Research Assistant</option>' +
                    '</select><br><br>' +
                    '<textarea id="return_remark" class="swal2-textarea" placeholder="Type your return reason..."></textarea>',
                showCancelButton: true,
                ...
                preConfirm: () => {
                    ...
                    if (!remark) {
                        Swal.showValidationMessage('You need to provide a return remark!');
                        return false;
                    }
                ...
                if (result.isConfirmed) {
                    ...
                    const remark = result.value.remark;
                    ...
                    Swal.fire('Cancelled', 'Return action has been cancelled.', 'info');
                }
            });
        });
    </script>
