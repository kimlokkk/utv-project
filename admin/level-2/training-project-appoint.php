    ...
    if (isset($_GET['id'])) {
        ...
        if ($update_result && $select_result && mysqli_num_rows($select_result) > 0) {
            ...
            if ($insert_result) {
                ...
                header("Location: training-project-info.php?update=appoint-success&id=$project_id");
                exit();
            }
        }
    
        // If any step fails, redirect with an error message (fallback)
        header("Location: training-project-info.php?update=appoint-fail&id=$project_id");
        exit();
    } else {
