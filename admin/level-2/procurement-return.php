<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        // Validasi input
        if (empty($_GET['project_id']) || empty($_GET['procurement_id']) || empty($_GET['project_no']) || empty($_GET['staff_id']) || empty($_GET['return_to'])) {
        ...
        $return_to = mysqli_real_escape_string($db, $_GET['return_to']);
        $return_remark = mysqli_real_escape_string($db, $_GET['return_remark']);
        $remark = "Procurement application has been returned - $return_remark ($staff_id)";
        ...
        try {
            ...
            $update_query = "UPDATE procurement
                             SET status = 'Returned by admin - $return_remark', return_to = '$return_to'
            ...
            echo json_encode(['success' => true, 'message' => 'Procurement has been returned and tracked!']);
        } catch (Exception $e) {
