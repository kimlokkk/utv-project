...
<body class="skin-green fixed-layout">
                                    ...
                                    <table class="table table-bordered table-striped" id="projectFundingTable">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    <?php if (stripos($status, 'Pending Approval') !== false) { ?>
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    $isReturned = stripos($status, 'Returned') !== false;
                                                    $isPendingApproval = stripos($status, 'Approved') !== false;
                                                    $isSendToBank = stripos($status, 'Send to bank') !== false;
                                                    $isPendingVerification = stripos($status, 'Pending Verification') !== false;
                                                    $disableButtons = $isRejected || $isReturned || $isPendingApproval || $isSendToBank || $isPendingVerification;
                                                    ?>
                                                    <button type="button" class="btn btn-success btn-sm" id="approveProjectFunding" 
                                                        data-application-id="<?php echo urlencode($application_id); ?>" 
    ...
    <script>
        $(document).on('click', '#rejectProjectFunding', function () {
            Swal.fire({
                title: 'Are you sure?',
                text: "Once return, you cannot undo this action!",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        ...
                        Swal.fire(
                            'Error!',
                            'An unexpected error occurred while handling the remarks.',
                            'error'
                        );
                    });
                } else {
        ...
        $(document).on('click', '#approveProjectFunding', function () {
            Swal.fire({
                ...
                if (result.value) { // Jika pengguna mengesahkan
                    ...
                    $.ajax({
                        url: 'project-funding-approve.php',
                    ...
                    Swal.fire(
                        'Cancelled',
                        'Project funding assistance application approval has been cancelled.',
                        'info'
                    );
                }
            });
