<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') { // Gunakan POST
        // Validasi input
        if (empty($_POST['projectId']) || empty($_POST['applicationId']) || empty($_POST['projectNo']) || empty($_POST['remark']) || empty($_POST['staffId'])) {
        ...
        try {
            ...
            $tracker_query = "INSERT INTO project_tracker (project_id, project_no, remark, date) 
                              VALUES ('$project_id', '$project_no', 'Project funding assistance application has been rejected by admin ($staff_id) - $remark', '$date')";
            ...
            echo json_encode(['success' => true, 'message' => "Project funding assistance application has been rejected !"]);
        } catch (Exception $e) {
