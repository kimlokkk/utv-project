<?php
    ...
    if (isset($_GET['id'])) {
        ...
        $update_query = "UPDATE project SET project_status = 'Appointed' WHERE id = '$project_id'";
        ...
        if ($update_result && $select_result && mysqli_num_rows($select_result) > 0) {
            ...
            $remark = "Project has been appointed ({$userData['staff_id']})";
            $date = date('Y-m-d');
            ...
            if ($insert_result) {
                ...
                header("Location: consultancy-project-info.php?update=appoint-success&id=$project_id");
                exit();
            }
        }
    
        // If any step fails, redirect with an error message (fallback)
        header("Location: consultancy-project-info.php?update=appoint-fail&id=$project_id");
        exit();
    } else {
