<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') { // Gunakan POST
        // Validasi input
        if (empty($_POST['projectId']) || empty($_POST['professionalId']) || empty($_POST['projectNo']) || empty($_POST['remark']) || empty($_POST['memberId']) || empty($_POST['staffId'])) {
        ...
        $member_id = mysqli_real_escape_string($db, $_POST['memberId']);
        ...
        try {
            ...
            $tracker_query = "INSERT INTO project_tracker (project_id, project_no, remark, date) 
                              VALUES ('$project_id', '$project_no', 'Professional fee application for $member_name has been rejected by admin ($staff_id); $remark', '$date')";
            ...
            echo json_encode(['success' => true, 'message' => 'Professional fee application has been rejected successfully!']);
        } catch (Exception $e) {
