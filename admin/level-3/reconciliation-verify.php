...
if ($_SERVER['REQUEST_METHOD'] === 'GET') { // Gunakan GET
    $staff_id_input = $_GET['staffId'] ?? ($_SESSION['user_data']['staff_id'] ?? '');

    // Validasi input
    if (empty($_GET['projectId']) || empty($_GET['applicationId']) || empty($_GET['applicationType']) || empty($_GET['projectNo']) || empty($staff_id_input)) {
