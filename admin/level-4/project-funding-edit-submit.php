...
function pfa_admin_edit_response($success, $message) {
    echo json_encode(['success' => $success, 'message' => $message]);
    exit;
}

if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    pfa_admin_edit_response(false, 'Invalid request method.');
}
...
if (
    ...
    pfa_admin_edit_response(false, 'Please complete all project funding assistance details.');
}

$application_id = mysqli_real_escape_string($db, $_POST['application_id']);
$app_query = "
    SELECT pfa.*, p.project_no
...
if (!$app_result || mysqli_num_rows($app_result) === 0) {
    pfa_admin_edit_response(false, 'This project funding assistance application cannot be edited by Level 3.');
}
...
for ($i = 0; $i < count($categories); $i++) {
    ...
    if ($category === '' || $item === '' || $quantity < 1 || $amount < 0) {
        pfa_admin_edit_response(false, 'Please complete all item rows correctly.');
    }
...
if (empty($valid_items)) {
    pfa_admin_edit_response(false, 'Please add at least one PFA item.');
}

if ($total_item_amount > (float)$expected_payment_amount) {
    pfa_admin_edit_response(false, 'Total item amount (RM ' . number_format($total_item_amount, 2) . ') must not be more than the expected payment from client (RM ' . number_format((float)$expected_payment_amount, 2) . ').');
}
...
try {
    ...
    if (!mysqli_query($db, $update_query)) {
        throw new Exception('Failed to update application: ' . mysqli_error($db));
    }
    ...
    foreach ($valid_items as $valid_item) {
        ...
        $item_query = "INSERT INTO project_funding_assistance_items (application_id, category, item, quantity, amount, date_create) VALUES ('$application_id', '$category', '$item', '$quantity', '$amount', '$date_now')";
    ...
    mysqli_query($db, "INSERT INTO project_tracker (project_id, project_no, remark, date) VALUES ('$project_id', '$project_no', 'Project funding assistance application has been edited by Level 3 and resubmitted for approval.', '$date_now')");

    mysqli_commit($db);
    pfa_admin_edit_response(true, 'Project funding assistance application updated and sent for approval.');
} catch (Exception $e) {
    mysqli_rollback($db);
    pfa_admin_edit_response(false, $e->getMessage());
}
?>
