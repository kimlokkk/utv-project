...
<body class="skin-green fixed-layout">
                                    ...
                                    <table class="table table-bordered table-striped" id="allowanceTable">
                                        ...
                                        <tbody>
                                            ...
                                            $query2 = "
                                                ...
                                                WHERE aa.status NOT LIKE '%project leader%'
                                                ORDER BY aa.id DESC";
                                            ...
                                            if (mysqli_num_rows($result2) > 0) {
                                                while ($row = mysqli_fetch_array($result2)) {
                                                    ...
                                                    $isReturnedToLevel4 = strcasecmp($return_to, 'Level 4') === 0 && (stripos($status, 'Rejected') !== false || stripos($status, 'Returned') !== false);
                                            ?>
                                                    <tr>
                                                        ...
                                                        <td class="text-center">
                                                            <?php
                                                            $isRejected = stripos($status, 'Rejected') !== false && !$isReturnedToLevel4;
                                                            $isPendingApproval = stripos($status, 'Pending Approval') !== false || stripos($status, 'Approved') !== false || stripos($status, 'Send to bank') !== false;
                                                            $disableButtons = $isRejected || $isPendingApproval;
                                                            ?>
                                                            <button type="button" class="btn btn-info btn-sm" id="verifyAllowance"
                                                                data-application-id="<?php echo urlencode($application_id); ?>"
                                                            ...
                                                            <button type="button" class="btn btn-danger btn-sm" id="rejectAllowance"
                                                                data-application-id="<?php echo urlencode($application_id); ?>"
                                                                data-project-id="<?php echo urlencode($project_id); ?>"
                                                                data-project-no="<?php echo urlencode($project_no); ?>"
                                                                data-admin-staff-id="<?php echo $userData['staff_id']; ?>"
    ...
    <script>
        $(document).on('click', '#rejectAllowance', function() {
            Swal.fire({
                title: 'Are you sure?',
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire({
                        ...
                        if (remarkResult.isConfirmed) {
                            ...
                            Swal.fire(
                                'Cancelled',
                                'You need to provide a remark to reject the allowance.',
                                'info'
                            );
                        }
                    }).catch((error) => {
                    ...
                    Swal.fire(
                        'Cancelled',
                        'Allowance/wages rejection has been cancelled.',
                        'info'
                    );
                }
            });
    ...
    <script>
        $(document).on('click', '#verifyAllowance', function() {
            Swal.fire({
                title: 'Are you sure?',
                ...
                if (result.value) { // Jika pengguna mengesahkan
                    ...
                    Swal.fire(
                        'Cancelled',
                        'Allowance/wages verification has been cancelled.',
                        'info'
                    );
                }
            });
