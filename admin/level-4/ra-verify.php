<?php
    ...
    if (isset($_POST['research_id'])) {
        ...
        // Update status
        $update_query = "UPDATE research_assistant SET status = 'Pending Approval', return_to = '' WHERE id = '$research_id'";
        ...
        if ($update_result && $select_result && mysqli_num_rows($select_result) > 0) {
            ...
            // Insert remark
            $remark = "$full_name has been verified by staff ID: $staff_id";
            ...
            if ($insert_result) {
                echo json_encode([
                    'success' => true,
                    'message' => "$full_name has been successfully verified."
                ]);
                exit;
            } else {
                echo json_encode([
