<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') { // Gunakan GET
        // Validasi input
        if (empty($_GET['projectId']) || empty($_GET['professionalId']) || empty($_GET['projectNo'])) {
        ...
        // ✅ Tambah di sini
        $get_member_query = "SELECT member_id FROM professional_fee_applications WHERE id = '$professional_id' LIMIT 1";
        $get_member_result = mysqli_query($db, $get_member_query);
        
        if (!$get_member_result || mysqli_num_rows($get_member_result) == 0) {
            echo json_encode(['success' => false, 'message' => 'Unable to retrieve member associated with the application.']);
            exit;
        }
        
        $member_row = mysqli_fetch_assoc($get_member_result);
        ...
        // Baru boleh buat remark
        $remark = "Professional fee application for $member_name has been submitted by project leader.";
        ...
        try {
            ...
            echo json_encode(['success' => true, 'message' => 'Professional fee application has been successfully submitted and tracked!']);
        } catch (Exception $e) {
