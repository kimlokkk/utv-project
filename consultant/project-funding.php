...
<?php
    ...
    $query = "SELECT 
                ...
                FROM project p
                INNER JOIN project_funding_assistance_applications pfa ON p.id = pfa.project_id
...
<body class="skin-blue fixed-layout">
                            ...
                            <div class="card-body">
                                <?php if ($hasApplications) { ?>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-striped" id="projectFundingTable">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?php echo date('d F Y', strtotime($expected_payment_date)); ?></td>
                                                    <td>
                                                        <?php
                                                        $isInternalLevel3Return = (stripos($status, 'Rejected') !== false || stripos($status, 'Returned') !== false) && strcasecmp($return_to, 'Level 3') === 0;
                                                        $displayStatus = $isInternalLevel3Return ? 'Pending Verification' : $status;
                                                        $statusText = htmlspecialchars($displayStatus);
                                                        $statusClass = 'badge-secondary'; // default
                                                    
                                                        if (stripos($displayStatus, 'Rejected') !== false || stripos($displayStatus, 'Returned') !== false) {
                                                            $statusClass = 'badge-danger';
                                                        } elseif (stripos($displayStatus, 'Pending Verification') !== false || stripos($displayStatus, 'Pending Approval') !== false) {
                                                            $statusClass = 'badge-warning';
                                                        } elseif (stripos($displayStatus, 'Approved') !== false || stripos($displayStatus, 'Send to bank') !== false) {
                                                    ...
                                                    <td class="text-center">
                                                        <?php if ((stripos($status, 'Rejected') !== false || stripos($status, 'Returned') !== false) && strcasecmp($return_to, 'Consultant') === 0) { ?>
                                                            <a href="project-funding-edit.php?applicationId=<?php echo urlencode($application_id); ?>" class="btn btn-warning btn-sm">Edit</a>
                                                        <?php } else { ?>
                                    ...
                                    <div class="text-center mt-4">
                                        <button class="btn btn-primary btn-lg" data-toggle="modal" data-target="#selectProjectModal">Create New Project Funding Assistance Application</button>
                                    </div>
                                <?php } else { ?>
                                    <!-- No Data Message -->
                                    <div class="text-center py-5">
                                        <h4>No Project Funding Assistance Applications Found</h4>
                    ...
                    <div class="modal-dialog modal-lg" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="viewDetailsModalLabel">Project Funding Assistance Details</h5>
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form id="selectProjectForm">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control select2" id="projectSelect" name="projectId" required style="width: 100%;">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-primary" id="proceedToProjectFundingApplication">Proceed</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        document.getElementById('proceedToProjectFundingApplication').addEventListener('click', function () {
            const projectId = document.getElementById('projectSelect').value;
            if (projectId) {
                window.location.href = `project-funding-application.php?projectId=${encodeURIComponent(projectId)}`;
            } else {
    ...
    <script>
        $(document).on('click', '.viewDetails', function () {
            ...
            $.ajax({
                url: 'get-project-funding-details.php',
                method: 'POST',
    ...
    <script>
        $(document).on('click', '#approveProjectFunding', function () {
            Swal.fire({
                title: 'Are you sure?',
                ...
                if (result.value) { // Jika pengguna mengesahkan
                    ...
                    console.log("Project Funding Assistance ID:", applicationId);
                    console.log("Project ID:", projectId);
                    console.log("Project No:", projectNo);
    
                    // Hantar permintaan AJAX ke server
                    $.ajax({
                        url: 'submit_project_funding.php',
    ...
    <script>
       $(document).on('click', '#rejectProjectFunding', function () {
            const applicationId = $(this).data('application-id');
            const projectId = $(this).data('project-id');
            const projectNo = $(this).data('project-no');
        
            Swal.fire({
                title: 'Are you sure you want to reject this project funding assistance application ?',
                input: 'textarea',
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: 'reject_project_funding.php',
                        method: 'GET',
                        data: {
                            applicationId: applicationId,
                            projectId: projectId,
                    ...
                    Swal.fire('Cancelled', 'Project funding assistance rejection has been cancelled.', 'info');
                }
            });
        });
    </script>
