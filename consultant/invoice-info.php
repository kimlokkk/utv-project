                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="card">
                                    <h3 class="card-header bg-info text-white">Invoice Info</h3>
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="card">
                                    <h3 class="card-header bg-info text-white">Invoice Documents</h3>
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group">
                        ...
                        <!-- Project Milestones -->
                        <?php if ($follow_milestone) { ?>
                        <!-- Project Milestones -->
                        <div class="card">
                            <h3 class="card-header bg-info text-white">Selected Project Milestones</h3>
                            <div class="card-body">
                                <p class="text-muted">
                                    This invoice follows project milestone. The milestone(s) below were selected during invoice submission.
                                </p>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-striped">
                                            ...
                                            <tbody>
                                                <?php 
                                                    $milestoneCounter = 1;
                                                    foreach ($invoice_milestones as $milestone) { 
                                                        $milestoneTitle = !empty($milestone['title']) ? $milestone['title'] : 'Milestone not found';
                                                        $milestoneDescription = !empty($milestone['description']) ? $milestone['description'] : 'No description available';
                                                        $milestoneValue = isset($milestone['value']) ? (float)$milestone['value'] : 0;
                                                        $milestoneStart = !empty($milestone['date_start']) ? date("j F Y", strtotime($milestone['date_start'])) : 'Not Available';
                                                        $milestoneEnd = !empty($milestone['date_end']) ? date("j F Y", strtotime($milestone['date_end'])) : 'Not Available';
                                                ?>
                                                <tr>
                                                    <td><?php echo $milestoneCounter++; ?></td>
                                                    <td><?php echo htmlspecialchars($milestoneTitle); ?></td>
                                                    <td><?php echo htmlspecialchars($milestoneDescription); ?></td>
                                                    <td>RM <?php echo number_format($milestoneValue, 2); ?></td>
                                                    <td><?php echo $milestoneStart; ?></td>
                                                    <td><?php echo $milestoneEnd; ?></td>
                                                </tr>
                                                <?php } ?>
                                            </tbody>
                                        </table>
                                ...
                                <?php } else { ?>
                                    <div class="alert alert-warning mb-0">
                                        This invoice is marked as following milestone, but no selected milestone record was found.
                                    </div>
                                <?php } ?>
                            </div>
                        </div>
                        ...
                        <!-- Invoice Amount -->
                        <div class="card">
                            <h3 class="card-header bg-info text-white">Invoice Amount</h3>
                            <div class="card-body"><div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <strong>Invoice amount will follow milestone?</strong>
                                            <h5><?php echo $follow_milestone ? 'Yes' : 'No'; ?></h5>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                        ...
                        <!-- Submit Buttons -->
                        <?php
                        $isNotLeader = ($userData['id'] != $leader_id);
                        ...
                        $isSentToBank = stripos($invoice_status, 'Send to bank') !== false;
                        $isPendingVerification = stripos($invoice_status, 'Pending Verification') !== false;
                        $isPendingLeaderReview = stripos($invoice_status, 'Pending Leader Review') !== false || stripos($invoice_status, 'Pending approval from project leader') !== false;
                        $disableButtons = $isNotLeader || !$isPendingLeaderReview || $isRejected || $isReturned || $isPendingVerification || $isApproved || $isSentToBank;
                        $isInvoiceLockedForEdit = $isPendingLeaderReview || $isPendingVerification || $isApproved || $isRejected || $isSentToBank;
                        $canEditInvoice = !$isInvoiceLockedForEdit && ((string)$userData['id'] === (string)$member_id || (string)$userData['id'] === (string)$leader_id);
                        ?>
                        <div class="row m-t-30 m-b-30">
                            <div class="col-md-12">
                                <?php if ($canEditInvoice) { ?>
                                <a href="invoice-edit.php?projectId=<?php echo urlencode($project_id); ?>&invoiceId=<?php echo urlencode($invoice_id); ?>" class="btn btn-lg btn-info">
                                    Edit Invoice
                                </a>&nbsp;&nbsp;
                                <?php } ?>
                                <button type="button" id="submitInvoice" class="btn btn-lg btn-success"
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) { // Guna .isConfirmed, bukan .value
                ...
                Swal.fire({
                    ...
                    text: 'Please wait while the invoice is sent for verification.',
                ...
                Swal.fire(
                    'Cancelled',
                    'Invoice submission has been cancelled.',
                    'info'
                );
            }
        });
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire({
                        ...
                        text: 'Please wait while the invoice rejection is saved.',
                        allowOutsideClick: false,
                        allowEscapeKey: false,
                        showConfirmButton: false,
                        didOpen: () => {
                    ...
                    Swal.fire('Cancelled', 'Invoice rejection has been cancelled.', 'info');
                }
            });
        });
    </script>
