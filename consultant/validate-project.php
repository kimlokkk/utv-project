...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $required_fields = [
        ...
        'quotation_doc' => 'Quotation Document',
        ...
        'client_pic' => 'Person in Charge (PIC)',
        'client_pic_email' => 'PIC Email',
        'client_pic_contact' => 'PIC Contact Number'
    ];
    ...
    // Validasi timeline projek
    $timeline_query = "SELECT COUNT(*) AS count FROM project_timeline WHERE project_id = '$project_id'";
    $timeline_result = mysqli_query($db, $timeline_query);

    if (!$timeline_result || mysqli_fetch_assoc($timeline_result)['count'] == 0) {
        echo json_encode([
            'success' => false,
            'message' => 'Project timeline data is missing. Please ensure all required timeline entries are completed.'
        ]);
        exit;
    }

    // Update status projek ke "Pending Verification" and clear previous return remarks
    $update_query = "UPDATE project
                     SET project_status = 'Pending Verification',
                         return_remark = ''
                     WHERE id = '$project_id'";
