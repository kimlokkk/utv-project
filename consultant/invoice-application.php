...
<?php
    ...
    $query = "SELECT 
            ...
            FROM project p
            INNER JOIN invoices i ON i.project_id = p.id
            INNER JOIN uitm_staff s ON i.member_id = s.id
            WHERE p.project_status IN ('Approved', 'Appointed')
              AND (
                    p.leader_id = '$user_id' OR 
                    EXISTS (
                        SELECT 1 FROM project_members_consultant pm 
...
<body class="skin-blue fixed-layout">
            ...
            <div class="container-fluid">
                ...
                <div class="row page-titles">
                    <div class="col-md-5 align-self-center">
                        <h4 class="text-themecolor">Invoice Application</h4>
                    </div>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <h3 class="card-header bg-info text-white">List of Invoice Application</h3>
                            <div class="card-body">
                                <?php if ($hasApplications) { ?>
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        } elseif (stripos($invoice_status, 'Pending Approval') !== false || stripos($invoice_status, 'Pending Verification') !== false || stripos($invoice_status, 'Pending Leader Review') !== false) {
                                                            $statusClass = 'badge-warning';
                                                        } elseif (stripos($invoice_status, 'Waiting Payment') !== false) {
                                                            $statusClass = 'badge-waiting-payment';
                                                        } elseif (stripos($invoice_status, 'Approved') !== false || stripos($invoice_status, 'Fully Paid') !== false || stripos($invoice_status, 'Send to bank') !== false) {
                                    ...
                                    <div class="text-center mt-4">
                                        <button class="btn btn-primary btn-lg" data-toggle="modal" data-target="#selectProjectModal">Create New Invoice Application</button>
                                    </div>
                                <?php } else { ?>
                                    <!-- No Data Message -->
                                    <div class="text-center py-5">
                                        <h4>No Invoice Applications Found</h4>
                                        <p>You have not created any invoice applications yet.</p>
                                        <button class="btn btn-primary btn-lg" data-toggle="modal" data-target="#selectProjectModal">Create New Invoice Application</button>
                                    </div>
                                <?php } ?>
                            </div>
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control select2" id="projectSelect" name="projectId" required>
                                            ...
                                            <?php 
                                                $user_id = $userData['id'];
                                                $projectQuery = "
                                                    SELECT p.id, p.project_no, p.project_title
                                                    FROM project p
                                                    WHERE p.project_status IN ('Approved', 'Appointed') AND (
                                                        p.leader_id = '$user_id'
                                                        OR EXISTS (
                                                            SELECT 1 FROM project_members_consultant pmc 
                                                            WHERE pmc.project_id = p.id AND pmc.member_id = '$user_id'
                                                        )
                                                    )
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-primary" id="proceedToInvoice">Proceed</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        $(function () {
            ...
            $('#selectProjectModal').on('shown.bs.modal', function () {
    ...
    <script>
        ...
        document.getElementById('proceedToInvoice').addEventListener('click', function () {
            ...
            if (projectId) {
                window.location.href = `apply-invoice.php?id=${encodeURIComponent(projectId)}`;
            } else {
