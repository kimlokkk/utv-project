...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['application_id'])) {
    ...
    // Query to get application + project details
    $query = "
        SELECT 
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        // Format date to 12 June 2025
        $expected_date = date('d F Y', strtotime($row['expected_payment_date']));
        ...
        // Application Info
        echo '<div class="section-title">Funding Assistance Details</div>';
        echo '<table class="elegant-table">';
        $is_internal_level3_return = (stripos($row['status'], 'Rejected') !== false || stripos($row['status'], 'Returned') !== false) && strcasecmp((string) $row['return_to'], 'Level 3') === 0;
        $display_status = $is_internal_level3_return ? 'Pending Verification' : $row['status'];

        echo '<tr><th>Expected To Receive Payment From Client Date</th><td>' . htmlspecialchars($expected_date) . '</td></tr>';
        ...
        if (!$is_internal_level3_return && !empty($row['return_remark'])) {
        ...
        if (!$has_items) {
            echo '<tr><td colspan="4" class="text-center">No funding items found.</td></tr>';
        } else {
