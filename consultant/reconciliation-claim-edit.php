...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $flow_result = mysqli_query($db, "
            SELECT rc.applicant_id, p.leader_id, p.project_no
            ...
            LIMIT 1
        ");
        if (!$flow_result || mysqli_num_rows($flow_result) === 0) {
            throw new Exception('Application not found.');
        }
        $flow_data = mysqli_fetch_assoc($flow_result);
        ...
        foreach (arc_edit_nested_files_from_request('proof_file') as $row_index => $row_files) {
            foreach ($row_files as $file) {
                $uploaded_name = arc_edit_upload_one_file(
                    $file,
                    $upload_dir,
                    ($flow_data['project_no'] ?? 'project') . '_' . ($flow_data['applicant_id'] ?? 'applicant') . '_proof'
                );
        ...
        $next_status = ((string)$flow_data['leader_id'] === (string)($userData['id'] ?? ''))
            ? 'Pending Verification'
            : 'Pending leader review';

        $update_parts = ["application_type = '$application_type'", "status = '$next_status'"];
        ...
        $column_check = mysqli_query($db, "SHOW COLUMNS FROM reconciliation_claim_applications LIKE 'remark_return'");
        if ($column_check && mysqli_num_rows($column_check) > 0) {
            $update_parts[] = "remark_return = ''";
        }
        ...
        foreach ($uploaded_file_names ?? [] as $uploaded_file_name) {
            $uploaded_path = ($upload_dir ?? 'project-documents/reconciliation-claim-receipts/') . $uploaded_file_name;
...
$application_query = "
    SELECT rc.*, p.project_no, p.project_title, p.project_leader AS project_leader_name, p.leader_id
    FROM reconciliation_claim_applications rc
    INNER JOIN project p ON rc.project_id = p.id
    WHERE rc.application_id = '$application_id'
    LIMIT 1
...
$current_user_id = (string)($userData['id'] ?? '');
$can_edit = ((string)$application['applicant_id'] === $current_user_id || (string)$application['leader_id'] === $current_user_id)
    && (stripos((string)$application['status'], 'Rejected') !== false || stripos((string)$application['status'], 'Returned') !== false);

if (!$can_edit) {
    echo '<script>alert("You are not allowed to edit this application."); window.location.href="reconciliation-claim.php";</script>';
    exit;
}
                                ...
                                <tbody>
                                    ...
                                    <tr data-row-index="<?php echo $index; ?>">
                                        ...
                                        <td>
                                            ...
                                            <input type="hidden" name="old_proof_file[]" class="old-proof-file" value="<?php echo htmlspecialchars(json_encode($proof_files)); ?>">
                                        ...
                                        <td class="proof-column">
                                            ...
                                            <div class="proof-links">
                                                ...
                                                <span class="proof-file-item" data-file="<?php echo htmlspecialchars($proof_file); ?>">
                                                    <a href="project-documents/reconciliation-claim-receipts/<?php echo htmlspecialchars($proof_file); ?>" target="_blank" class="btn btn-outline-primary btn-sm">
                                                        File <?php echo $file_index + 1; ?>
                                                    </a>
                                                    <button type="button" class="btn btn-outline-danger btn-sm removeExistingProof">Remove</button>
                                                </span>
                                                <?php } ?>
                                            </div>
                                            <?php } ?>
...
function rowHtml(rowIndex) {
    return '<tr data-row-index="' + rowIndex + '">' +
        ...
        '<td>' +
            ...
            '<input type="hidden" name="old_proof_file[]" class="old-proof-file" value="[]">' +
...
$(document).on('click', '.removeExistingProof', function(){
    var row = $(this).closest('tr');
    var item = $(this).closest('.proof-file-item');
    var file = item.data('file');
    var hiddenInput = row.find('.old-proof-file');
    var files = [];
    ...
    files = files.filter(function(existingFile) {
        return existingFile !== file;
    });
    hiddenInput.val(JSON.stringify(files));
    item.remove();
});
