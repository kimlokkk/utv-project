<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        // Validasi input
        if (empty($_GET['projectId']) || empty($_GET['applicationId']) || empty($_GET['reject_remark']) || empty($_GET['projectNo'])) {
        ...
        try {
            ...
            $update_query = "UPDATE project_funding_assistance_applications
                             SET status = 'Rejected',
                                 ...
                                 return_remark = '$reject_remark'
                             WHERE id = '$application_id'";
            $update_result = mysqli_query($db, $update_query);
    
            if (!$update_result) {
                throw new Exception('Failed to update project funding assistance status.');
            }
