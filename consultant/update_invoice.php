...
$project_check_query = "SELECT leader_id, project_no FROM project WHERE id = '$project_id' LIMIT 1";
...
$is_invoice_owner = (string)$invoice_data['member_id'] === $current_user_id;
$is_project_leader = (string)$project_data['leader_id'] === $current_user_id;
$is_returned_invoice = stripos((string)$invoice_data['invoice_status'], 'Returned') !== false;
$is_locked_for_edit = stripos((string)$invoice_data['invoice_status'], 'Pending Verification') !== false
    || stripos((string)$invoice_data['invoice_status'], 'Pending Leader Review') !== false
    || stripos((string)$invoice_data['invoice_status'], 'Pending approval from project leader') !== false
    || stripos((string)$invoice_data['invoice_status'], 'Approved') !== false
    || stripos((string)$invoice_data['invoice_status'], 'Rejected') !== false
    || stripos((string)$invoice_data['invoice_status'], 'Send to bank') !== false;
$member_id = mysqli_real_escape_string($db, $invoice_data['member_id']);

if ((!$is_invoice_owner && !$is_project_leader) || $is_locked_for_edit) {
    echo json_encode([
        'success' => false,
...
$status_update_sql = $is_returned_invoice ? ",
                invoice_status = 'Pending Verification',
                return_remark = ''" : "";
...
try {
    ...
    */
    ...
    $consultant_name = isset($userData['fullname']) ? $userData['fullname'] : ($userData['name'] ?? $current_user_id);
    $consultant_name = mysqli_real_escape_string($db, $consultant_name);
    $project_no_sql = mysqli_real_escape_string($db, $project_no);
    $tracker_text = $is_returned_invoice
        ? "Invoice application has been updated and resubmitted by consultant ($consultant_name)."
        : "Invoice application has been updated by consultant ($consultant_name).";
    $tracker_description = mysqli_real_escape_string($db, $tracker_text);
    $tracker_query = "
        INSERT INTO project_tracker (project_id, project_no, remark, date)
        VALUES ('$project_id', '$project_no_sql', '$tracker_description', NOW())
    ";
    ...
    echo json_encode([
        'success' => true,
        'message' => $is_returned_invoice ? 'Invoice has been successfully updated and resubmitted!' : 'Invoice has been successfully updated!',
