...
<body class="skin-blue fixed-layout">
                            ...
                            <div class="card-body">
                                <?php if ($hasApplications) { ?>
                                            ...
                                            <tbody>
                                                <?php
                                                    ...
                                                    $query2 = "SELECT 
                                                                ...
                                                                FROM project p
                                                                INNER JOIN procurement pr ON p.id = pr.project_id
                                                                LEFT JOIN uitm_staff u ON pr.member_id = u.id
                                                                WHERE p.leader_id = '$user_id'
                                                                   OR EXISTS (
                                                                       SELECT 1 
                                                                       FROM project_members_consultant pm 
                                    ...
                                    <div class="text-center mt-4">
                                        <button class="btn btn-primary btn-lg" data-toggle="modal" data-target="#selectProjectModal">Create New Procurement Application</button>
                                    </div>
                                <?php } else { ?>
                                    <!-- No Data Message -->
                                    <div class="text-center py-5">
                                        <h4>No Procurement Applications Found</h4>
                                        <p>You have not created any procurement applications yet.</p>
                                        <button class="btn btn-primary btn-lg" data-toggle="modal" data-target="#selectProjectModal">Create New Procurement Application</button>
                                    </div>
                                <?php } ?>
                            </div>
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                                ...
                                <form id="selectProjectForm">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="projectSelect" name="projectId" required>
                                            ...
                                            <?php 
                                            $projectQuery = "SELECT p.id, p.project_no, p.project_title
                                                    FROM project p
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-primary" id="proceedToProcurement">Proceed</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        document.getElementById('proceedToProcurement').addEventListener('click', function () {
            const projectId = document.getElementById('projectSelect').value;
            if (projectId) {
                window.location.href = `procurement-application.php?projectId=${encodeURIComponent(projectId)}`;
            } else {
