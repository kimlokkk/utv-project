<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') { // Gunakan GET
        // Validasi input
        if (empty($_GET['projectId']) || empty($_GET['applicationId']) || empty($_GET['projectNo']) || empty($_GET['applicationType'])) {
        ...
        $remark = "$application_type application has been approved by project leader.";
        ...
        try {
            ...
            echo json_encode(['success' => true, 'message' => "$application_type application has been successfully submitted and tracked!"]);
        } catch (Exception $e) {
